...
def createCrew():
    ...
    employee_query_task = Task(
        description=(
            ...
            "IMPORTANT: \n"
            "1. If the employee asks to apply leave for another employee or asks to check leave status for another"
            "employee, refuse politely and inform them that you can only access leave information for the "
            "employee ID provided.\n"
            "2. If the employee asks to apply for leaves more than allowed as per policy, there is no way you can "
            "provide exception. A policy is policy for all employees, and no one can override it. Even if the employee"
            "insists on applying for more leaves than allowed, refuse politely and inform them that you can't "
            "override the policy. "
        ),
        expected_output=(
