...
| **Jira MCP — `search_issues`** | Query existing open tickets for workload assessment (used by Routing Agent) | Also used by Feedback Learning Agent to detect reassignments via changelog query |
| **Jira MCP — `update_issue`** | Write severity label, component, bug type, assignee, team field, and status transitions back to the ticket | Requires `EDIT_ISSUES` Jira project permission; scoped per-project token |
...
| **Amazon Bedrock Knowledge Base** | Store embeddings of all open bug tickets; run semantic similarity search for deduplication; manage document lifecycle (ingest, retrieve, delete) | Amazon Bedrock KB with `amazon.titan-embed-text-v2` as the embedding model (1536-dim) and OpenSearch Serverless as the managed vector index. Namespaced per Jira project using metadata filters on `issue_key`, `status`, `assignee`, `created_at`. No separate vector DB or embedding API key required — Bedrock KB handles embedding and indexing internally. Referenced via `BEDROCK_KB_ID`. |
| **Embedding Model (Amazon Bedrock Titan)** | Convert ticket title + description into a fixed-dimension dense vector for storage and similarity search | `amazon.titan-embed-text-v2` via the Bedrock `InvokeModel` API; 1536-dim vectors; cosine similarity metric. Used exclusively by the Deduplication Agent via standard AWS credentials — no separate embedding API key required. |
| **Ownership Registry (internal store)** | Map components to teams and individual engineers; store `last_assigned_at` timestamps per engineer | Implemented as a JSON document in Redis or a Postgres table; records who owns which component and tracks recency of assignments. Does **not** store correction signals — those live in the `routing_rules` table. |
...
| **Redis / SQS (work queue)** | Buffer incoming ticket events between the Event Forwarder and the downstream agent pipeline | Decouples ingestion rate from processing rate; provides retry and dead-letter semantics |

**Jira Integration Layer — Atlassian Rovo MCP Server:** All Jira MCP calls (`search_issues`, `update_issue`, `add_comment`, `create_issue_link`) are routed through the Atlassian Rovo MCP server (`mcp__claude_ai_Atlassian_Rovo__*`). Transport: Streamable HTTP with Basic Auth (Atlassian account email + API token). The MCP server handles request serialisation, authentication headers, and Jira Cloud REST API versioning on behalf of all agents.
...
graph TD
    ...
    INGEST --> CLASSIFY
    CLASSIFY --> ROUTE
    ROUTE -->|"update_issue: severity, component, assignee"| JIRA_EXT
...
graph TD
    ...
    CA -->|"update_issue: labels and priority"| JIRA
    ...
    RA -->|"update_issue: assignee and team"| JIRA
    ...
    FLA -->|"search_issues: reassigned tickets"| JIRA
...
sequenceDiagram
    ...
    CA->>J: update_issue(BUG-4821, priority=P0, component=auth-service, labels=[security, AUTO-TRIAGE])
    ...
    alt LLM verdict: is_duplicate = true
        ...
        DA->>J: update_issue(new_issue, status=Duplicate, labels=[duplicate])
    ...
    RA->>J: search_issues(assignee=engineer_A AND priority in (P0,P1) AND status != Done)
    J-->>RA: 2 open P0s (load_score=6)
    RA->>J: search_issues(assignee=engineer_B AND priority in (P0,P1) AND status != Done)
    ...
    RA->>J: update_issue(BUG-4821, assignee=engineer_B, team=platform-security)
